...
public class OVRRaycaster : GraphicRaycaster, IPointerEnterHandler
{
        ...
        bool checkForBlocking)
    {
        ...
        {
            var go = m_RaycastResults[index].graphic;
            ...
            // Ignore points behind us (can happen with a canvas pointer) Or graphics that ignore raycasting
            if (eventCamera.transform.InverseTransformPoint(m_RaycastResults[index].worldPos).z <= 0 || !go.raycastTarget)
            {
                appendGraphic = false;
            }
